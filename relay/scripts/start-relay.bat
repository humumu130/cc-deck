...
rem  - Token priority: env CCR_TOKEN > data\relay-token.txt > relay 首启随机生成（落 data\token，开源安全：不写死通用口令）
...
rem 开源安全（#22）：不写死 devtoken——CCR_TOKEN 缺省时由 relay 核心首启随机生成并落盘 data\token
