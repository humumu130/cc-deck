...
export const VERSION_NOTES: string[] = [
  "重大提速：手机发消息到电脑立刻提交，不再滞留输入框（最长可达 1 分钟的延迟根除）",
  "断网重连大幅提速：Wi-Fi 断开后数秒内切回云桥连接",
  "会话异常断开（死机/重启）不再丢卡片：点一下即可远程恢复继续对话",
  "电脑端主动退出的会话自动从列表清除，不再堆积",
  "模型别名接入时正确显示真实模型名，版本号旁显示通道标识（快照版/测试版）",
];
