...
// 话术纪律（2026-09-19 用户定稿口径，0.6.0 起执行）：素材 = 上一正式版之后的全部
// 提交按功能聚合；站在产品/用户视角（不写任务编号与内部机制名）；UI 类调整合并
// 一条内简单列举，具体功能项各自单列；不啰嗦但不埋没亮点。上一版（0.5.2）历史
// 条目随 0.6.0 清出（关于页只展示当前版本特性）。
export const VERSION_NOTES: string[] = [
  "全新「输出物」看板：会话产出的文档、报告自动归集成清单，手机、电脑、网页三端同步查看，交付进度一目了然（默认关闭，可在设置开启）",
  "输出物远程查看：在手机上点开任意输出物即可实时拉取预览——文本、图片直接看，Word、PPT 等复杂格式下载后交给手机上的应用打开，文件全程留在自己电脑上",
  "无人值守自动恢复：电脑重启后托管会话自动拉起继续跑，夜里挂机的任务不再需要人工干预",
  "会话卡死自愈：流式中断的僵死会话自动检测、自动恢复，长任务更省心",
  "手机可向会话发送文件：文档、表格、压缩包即选即发，电脑上直接可用（与发图同入口）",
  "挂机通知更可靠：修复息屏后任务完成通知偶发中断，新增后台保活一键豁免入口",
  "远程审批开关：外部会话在手机/网页一键打开「审批」，电脑上的权限确认直接转到手机上点",
  "权限模式一屏直选：标准/自动/规划/跳过四档带说明任选，危险档需二次确认，手机与桌面体验统一",
  "修复：上下文占用长期显示 100% 的统计失真，占用恢复真实水位",
  "修复：外部会话回复被拆成多条碎片刷屏、进行中的回复电脑端看不到",
  "下载更新更可靠：公司网络下检查更新与下载全面修复，不再「下载到 99% 重来」；测试/快照包不再误提示升级",
  "一批稳定性修复：审批弹窗偶发不出、恢复后消息滞留、手动命名的会话名丢失、中文标题乱码、会话列表堆积无效卡片、桌面端后台服务意外退出自动重拉等",
  "UI 与交互打磨：输入框多行自动增高、键盘弹起时对话整体上移、发送键与浅色主题风格统一、桌面会话卡等高、浮层不再互相遮挡、图标与提示文案统一等",
];
