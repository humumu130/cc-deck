...
// #313 本版特性摘要（关于弹窗分组展示），随版本发布同步维护。
// 更新说明军规（2026-09-21 用户拍板，权威全文见仓库 CLAUDE.md「更新说明军规」节）：
// 弹窗是橱窗不是仓库——① 动词开头，一条 = 做了什么 + 用户得到什么；② 主句一行
// ≤35 字，括号补充降级 note 灰字次行；③ 三组分组（新增/优化/修复），杂项修复打包
// 一条；④ 只写用户可感知结果，任务号/内部机制名不进弹窗；⑤ 总数 ≤8、单组 ≤4、
// 亮点置顶。全量细节进 GitHub Release 正文（弹窗只留「查看完整变更」外链）。
// test/snap 通道 notes 允许提交聚合机器生成；正式版必须人工收敛到军规内。
...
// 「查看完整变更」外链（关于弹窗/军规：全量细节不进弹窗，去仓库页）：test 通道指
// dev 提交历史（test 包无独立 Release），release/snap 指 Release 页
export function releasePageUrl(): string {
  return channelOf(currentVersion()) === "test" ? GH_COMMITS_PAGE : GH_RELEASE_PAGE;
}
